...
describe Relationship::Iterator, '#each' do
  let(:source_mapper)     { mock_mapper(source_model, source_attributes).new(ROM_ENV, relation) }
  let(:target_mapper)     { mock_mapper(target_model, target_attributes).new(ROM_ENV, orders_relation) }
