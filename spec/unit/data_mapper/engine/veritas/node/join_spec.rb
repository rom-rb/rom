...
describe Engine::Veritas::Node, '#join' do
  ...
  let(:source_aliases)  { Relation::Header.new(source_a_index, source_r_index) }
  let(:target_aliases)  { Relation::Header.new(target_a_index, target_r_index) }
  let(:source_a_index)  { Relation::Header::AttributeIndex.new({ attribute_alias(:id, :users)     => attribute_alias(:id, :users)     }, strategy) }
  let(:target_a_index)  { Relation::Header::AttributeIndex.new({ attribute_alias(:id, :addresses) => attribute_alias(:id, :addresses) }, strategy) }
