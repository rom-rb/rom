...
describe Graph::Node, '#join' do
  ...
  let(:source_header)    { Header.new(source_a_index, source_r_index) }
  let(:target_header)    { Header.new(target_a_index, target_r_index) }
  let(:source_a_index)   { Header::AttributeIndex.new(source_a_entries, strategy) }
  let(:source_a_entries) { { attribute_alias(:id, :users) => attribute_alias(:id, :users) } }
  let(:target_a_index)   { Header::AttributeIndex.new(target_a_entries, strategy) }
  let(:target_a_entries) { { attribute_alias(:id, :addresses) => attribute_alias(:id, :addresses) } }
  let(:source_r_index)   { Header::RelationIndex.new(:users     => 1) }
  let(:target_r_index)   { Header::RelationIndex.new(:addresses => 1) }
