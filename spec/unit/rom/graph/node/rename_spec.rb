...
describe Graph::Node, '#rename' do
  ...
  let(:header)           { Header.new(attribute_index, relation_index) }
  let(:attribute_index)  { Header::AttributeIndex.new(initial_entries, strategy) }
  let(:initial_entries)  { { attribute_alias(:id, :users) => attribute_alias(:id, :users) } }
  let(:relation_index)   { Header::RelationIndex.new(:users => 1) }
