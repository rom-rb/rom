...
describe Graph, '#build_edge' do
  ...
  context "when no edge with the same name is included" do
    it "delegates to Graph::Edge.new" do
      Graph::Edge.should_receive(:new).with(name, left, right)
      subject
    end
  end
  ...
  context "when an edge with the same name is included" do
    ...
    it "does not delegate to Graph::Edge.new" do
      Graph::Edge.should_not_receive(:new).with(name, left, right)
      subject
    end
  end
end
