...
RSpec.describe ROM::Plugins::Command::Timestamps do
  ...
  shared_examples_for "a command setting timestamps" do
    let(:user_command) { users[:create] }
    let(:result) { user_command.call(name: "Jane", email: "jane@doe.org") }
  ...
  shared_examples_for "a command setting datestamp" do
    ...
    it "applies datestamps by default" do
      expect(Date.parse(result[:written].to_s)).to eql(Date.today)
    end
  end
  ...
  it "only updates specified timestamps" do
    initial = users[:create].call(name: "Jane", email: "jane@doe.org")
    initial_updated_at = initial[:updated_at]

    updated = users[:update].call(name: "Jane Updated").first

    expect(updated[:created_at]).to eql(initial[:created_at])
    expect(updated[:updated_at]).not_to eql(initial_updated_at)
  end

  it "allows overriding timestamps" do
    tomorrow = (Time.now + (60 * 60 * 24))

    users[:create].call(name: "Jane", email: "jane@doe.org")
    updated = users[:update].call(name: "Jane Updated", updated_at: tomorrow).first
  ...
  it "works with chained commands" do
    ...
    create_note = users[:create_with_task].curry(name: "Jane")
