...
describe 'Reading relations' do
  ...
  it 'exposes a relation reader' do
    ...
    expect(user).to be_an_instance_of(ROMSpec::User)
  ...
  it 'maps grouped relations' do
    ...
    ROMSpec::User.send(:include, Equalizer.new(:name, :email))
    ...
    expect(user).to eql(
      ROMSpec::User.new(name: "Jane", email: "jane@doe.org")
    )
  ...
  it 'maps wrapped relations' do
    ...
    setup.mappers do
      ...
      define(:with_task, parent: :users) do
        model name: 'ROMSpec::UserWithTask'
    ...
    ROMSpec::User.send(:include, Equalizer.new(:name, :email))
    ROMSpec::UserWithTask.send(:include, Equalizer.new(:name, :email, :task))
